...
async function expireEndedAdvertListingsIfDue() {
    ...
    if (!(expireEndedAdvertListingsIfDue.__lastAt)) {
        expireEndedAdvertListingsIfDue.__lastAt = 0;
    }
    if (Date.now() - expireEndedAdvertListingsIfDue.__lastAt < TWO_MINUTES_MS)
        return;
...
async function runAdvertAnalyticsRollupIfDue() {
    ...
    if (Date.now() - lastAdvertRollupSweepAt < SIX_HOURS_MS)
        return;
