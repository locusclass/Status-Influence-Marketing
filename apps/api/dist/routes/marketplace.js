...
import { PRIME_REQUEST_ACCESS_PASS_PLANS, PRIME_REQUEST_PRICING, PRIME_REQUEST_UNLOCK_PRICE_UGX, debitPrimeRequestWallet, ensurePrimeRequestsSchema, expireOldPrimeRequests, fullPrimeRequestDto, getActivePrimeRequestPass, hasPrimeRequestAccess, insertPrimeRequestAccessPass, insertPrimeRequestUnlock, loadPrimeRequest, normalizePrimeRequestCreateInput, normalizePrimeRequestPassType, publicPrimeRequestDto, validatePrimeRequestCreateInput, } from '../services/primeRequests.js';
// ─────────────────────────────────────────────
// HELPERS
// ─────────────────────────────────────────────
...
function normalizeQueryText(value, max = 120) {
...
async function ensureContentBlocksColumn(client) {
    if (schemaEnsured)
        return;
...
export async function marketplaceRoutes(app) {
    ...
    app.get('/marketplace/categories', async (_request, reply) => {
        try {
            const rows = await query(`SELECT id, slug, name, icon, sort_order
    ...
    app.post('/marketplace/prime-requests', async (request, reply) => {
        ...
        try {
            ...
            const created = await withTransaction(async (client) => {
                ...
                const authUserId = String(request.user?.sub ?? '').trim() || null;
                const res = await client.query(`INSERT INTO prime_requests
    ...
    app.get('/marketplace/prime-requests/:id/full', {
        ...
        try {
            const result = await withTransaction(async (client) => {
                ...
                if (!row)
                    return { error: 'prime_request_not_found' };
    ...
    app.get('/marketplace/categories/:categoryId/listing-types', async (request, reply) => {
        ...
        try {
         ...
         ORDER BY sub.sort_order ASC, lt.sort_order ASC, lt.name ASC`, [categoryId]);
            reply.header('Cache-Control', 'public, max-age=60, stale-while-revalidate=120');
    ...
    app.get('/marketplace/listings', async (request, reply) => {
        ...
        try {
          ...
          LIMIT $${limitN} OFFSET $${offsetN}`, dataParams),
    ...
    app.get('/marketplace/listings/:slug', async (request, reply) => {
        ...
        WHERE al.slug = $1
          ...
          AND al.access_state = 'PUBLIC'`, [slug]);
            if (!rows[0])
                return reply.code(404).send({ error: 'listing_not_found' });
            const listing = rows[0];
            const media = await query(`SELECT id, media_pack, media_type, url, thumbnail_url, file_name, sort_order
         FROM advert_media WHERE listing_id = $1
         ORDER BY CASE media_pack WHEN 'AMBASSADOR_PACK' THEN 0 ELSE 1 END, sort_order ASC`, [listing.id]);
    ...
    app.post('/marketplace/listings', {
        ...
        const userId = String(request.user?.sub ?? request.user?.id ?? '').trim();
        if (!userId)
            return reply.code(401).send({ error: 'unauthorized' });
        let businessId = userId;
        try {
            // Check user role
            const userRows = await query(`SELECT id, role FROM users WHERE id = $1 LIMIT 1`, [userId]);
        ...
        let vendorId = null;
        try {
            const vendorRows = await query(`SELECT id FROM vendor_profiles WHERE user_id = $1 AND status = 'ACTIVE' LIMIT 1`, [businessId]);
            if (!vendorRows[0]) {
        ...
        try {
            ...
            await withTransaction(async (client) => {
                ...
                const inserted = await client.query(`INSERT INTO advert_listings (
          ...
          RETURNING *`, [
                ...
                if (body.images.length > 0) {
                    ...
                    await client.query(`INSERT INTO advert_media (id, listing_id, media_pack, media_type, url, sort_order)
             VALUES ${imgPlaceholders.join(',')}`, imgParams);
                }
            });
    ...
    app.patch('/marketplace/listings/:slug', {
        ...
        const body = parsed.data;
        try {
            const existingRows = await query(`SELECT id, business_id FROM advert_listings WHERE slug = $1 LIMIT 1`, [slug]);
            ...
            if (body.summary !== undefined)
                addField('summary', body.summary);
            if (body.description !== undefined)
                addField('description', body.description);
            if (body.price !== undefined)
                addField('price', body.price ?? null);
            if (body.currency !== undefined)
                addField('currency', body.currency);
            if (body.is_negotiable !== undefined)
                addField('is_negotiable', body.is_negotiable);
            if (body.location_text !== undefined)
                addField('location_text', body.location_text ?? null);
            if (body.cta_whatsapp !== undefined)
                addField('cta_whatsapp', body.cta_whatsapp ?? null);
            if (body.cta_phone !== undefined)
                addField('cta_phone', body.cta_phone ?? null);
            if (body.cta_email !== undefined)
                addField('cta_email', body.cta_email ?? null);
            if (body.cta_url !== undefined)
                addField('cta_url', body.cta_url ?? null);
            if (body.content_blocks !== undefined)
                addField('content_blocks', JSON.stringify(body.content_blocks));
            sets.push(`updated_at = NOW()`);
            if (sets.length > 1) {
                vals.push(listing.id);
                await query(`UPDATE advert_listings SET ${sets.join(', ')} WHERE id = $${vals.length}`, vals);
            }
            if (body.images !== undefined) {
                await withTransaction(async (client) => {
                    await client.query(`DELETE FROM advert_media WHERE listing_id = $1 AND media_pack = 'PRODUCT_GALLERY'`, [listing.id]);
                    if (body.images.length > 0) {
                        const imgParams = [];
                        const imgPlaceholders = body.images.map((img, i) => {
                            const b = i * 6;
    ...
    app.post('/marketplace/inquiries', async (request, reply) => {
        ...
        try {
            ...
            if (!vendorRows[0])
                return reply.code(404).send({ error: 'vendor_not_found' });
            if (listing_id) {
                const listingRows = await query(`SELECT id FROM advert_listings
           WHERE id = $1 AND vendor_id = $2 AND status = 'ACTIVE' AND access_state = 'PUBLIC'
           LIMIT 1`, [listing_id, vendor_id]);
                if (!listingRows[0])
                    return reply.code(400).send({ error: 'listing_vendor_mismatch' });
            }
