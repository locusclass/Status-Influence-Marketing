...
function isMimeAllowed(mime, purpose) {
...
async function checkReadAccess(requestUserId, requestUserRole, assetRow) {
    ...
    const ownerUserId = String(assetRow.owner_user_id ?? '');
    const campaignId = assetRow.campaign_id ?? null;
    ...
    if (!PROOF_PURPOSES.has(purpose) && campaignId) {
        const campaignRes = await pool.query('SELECT business_id FROM campaigns WHERE id = $1 LIMIT 1', [campaignId]);
        const businessId = campaignRes.rows[0]?.business_id;
        if (businessId && String(businessId) === requestUserId)
            return 'allow';
    }
    // Admin / super admin can always read.
    if (['ADMIN', 'SUPER_ADMIN'].includes(requestUserRole.toUpperCase()))
        return 'allow';
...
export async function uploadRoutes(app) {
    ...
    app.post('/uploads/sign', { preHandler: [app.authenticate] }, async (request, reply) => {
        ...
        const { file_name, mime_type, upload_purpose, campaign_id } = body;
        ...
        if (campaign_id) {
            const campRes = await pool.query('SELECT business_id, assigned_ambassador_id FROM campaigns WHERE id = $1 LIMIT 1', [campaign_id]);
    ...
    app.post('/uploads/:id', async (request, reply) => {
        ...
        const assetRes = await pool.query(`SELECT id, owner_user_id, upload_purpose, processing_status
       FROM media_assets WHERE id = $1 AND deleted_at IS NULL LIMIT 1`, [fileId]);
        const assetRow = assetRes.rows[0];
        // Determine size limit (from DB purpose if available, else basic rule).
        const purpose = assetRow?.upload_purpose ?? (mime.startsWith('video/') ? 'campaign_video' : 'campaign_gallery');
        ...
        if (isMultipart) {
            ...
            if (!isMimeAllowed(data.mimetype, purpose))
                return reply.code(400).send({ error: 'invalid_mime' });
            if (data.mimetype !== mime)
                return reply.code(400).send({ error: 'mime_mismatch' });
            ...
            if (!headerMime || !isMimeAllowed(headerMime, purpose))
                return reply.code(400).send({ error: 'invalid_mime' });
            if (headerMime !== mime)
                return reply.code(400).send({ error: 'mime_mismatch' });
        ...
        try {
            ...
            if (message.includes('firebase_storage_not_configured'))
                return reply.code(503).send({ error: 'firebase_storage_not_configured' });
            if (message.includes('firebase_access_token'))
                return reply.code(503).send({ error: 'firebase_access_token_failed' });
            if (message.includes('firebase_client_init_failed'))
                return reply.code(503).send({ error: 'firebase_client_init_failed' });
            if (message.includes('firebase_storage_upload_failed'))
                return reply.code(502).send({ error: 'firebase_storage_upload_failed', detail: message });
            throw error;
        }
        ...
        if (assetRow) {
         ...
         SET processing_status = 'uploaded',
             ...
             file_size_bytes = $3,
             updated_at = now()
         WHERE id = $1`, [fileId, objectName, total || null]);
    ...
    app.get('/uploads/media/:id', { preHandler: [app.authenticate] }, async (request, reply) => {
        ...
        const variant = String(request.query.variant ?? 'original'); // original | optimized | thumbnail
        const assetRes = await pool.query(`SELECT * FROM media_assets WHERE id = $1 AND deleted_at IS NULL LIMIT 1`, [assetId]);
    ...
    app.delete('/uploads/media/:id', { preHandler: [app.authenticate] }, async (request, reply) => {
        ...
        const userId = String(request.user?.sub ?? '').trim();
        const userRole = String(request.user?.role ?? '').trim();
        const isObserver = request.user?.is_observer === true;
        if (isObserver)
            return reply.code(403).send({ error: 'observer_cannot_mutate' });
        const assetRes = await pool.query(`SELECT id, owner_user_id, original_storage_path, upload_purpose
       FROM media_assets WHERE id = $1 AND deleted_at IS NULL LIMIT 1`, [assetId]);
        const asset = assetRes.rows[0];
        if (!asset)
            return reply.code(404).send({ error: 'not_found' });
       ...
       WHERE id = $1`, [assetId]);
        ...
        if (asset.original_storage_path) {
            await pool.query(`INSERT INTO job_queue (id, job_type, payload, status, run_at)
         VALUES (gen_random_uuid(), 'CLEANUP_MEDIA', $1, 'QUEUED', now() + interval '5 minutes')`, [JSON.stringify({ asset_id: assetId, object_name: asset.original_storage_path })]);
        }
        return { deleted: true };
    });
    ...
    app.get('/uploads/media/:id/status', { preHandler: [app.authenticate] }, async (request, reply) => {
        const assetId = request.params.id;
        const userId = String(request.user?.sub ?? '').trim();
        const assetRes = await pool.query(`SELECT id, owner_user_id, processing_status, moderation_status, verification_status,
