...
import { buildAuthClaims } from '../../services/roles.js';
import { applySchema, getTestPool } from '../../../test/db.js';
...
async function insertCountry(code, name) {
    const result = await pool.query(`INSERT INTO countries (name, code, status) VALUES ($1, $2, 'ACTIVE') RETURNING *`, [name, code]);
    return result.rows[0];
}
async function insertUser(input) {
    const result = await pool.query(`INSERT INTO users (full_name, email, phone, password_hash, role, active_role, admin_role, country, preferred_currency, country_id, status, max_status_viewers_12h)
    ...
    if (input.role === 'ADMIN' && input.admin_role) {
        await pool.query(`INSERT INTO admin_users (user_id, admin_role, is_observer, status) VALUES ($1, $2, $3, 'ACTIVE') ON CONFLICT (user_id) DO UPDATE SET admin_role=$2, is_observer=$3`, [user.id, input.admin_role, input.is_observer ?? false]);
    }
    return user;
}
async function insertProofWithSession(businessId, ambassadorId) {
    const campaignRes = await pool.query(`INSERT INTO campaigns (business_id, title, platform, payout_amount, budget_total, media_type, media_url, start_date, end_date, visibility, execution_mode, status)
    ...
    const sessionRes = await pool.query(`INSERT INTO verification_sessions (user_id, campaign_id, platform, challenge_code, challenge_phrase, script, expires_at, status)
     VALUES ($1, $2, 'WHATSAPP_STATUS', 'TEST123', 'blue mango', '[]'::jsonb, now() + interval '1 hour', 'ACTIVE') RETURNING *`, [ambassadorId, campaign.id]);
    const session = sessionRes.rows[0];
    const proofRes = await pool.query(`INSERT INTO proofs (session_id, user_id, video_url, status, verification_status, meta)
     VALUES ($1, $2, '/uploads/files/test.mp4', 'PENDING', 'MANUAL_REVIEW', '{}'::jsonb) RETURNING *`, [session.id, ambassadorId]);
...
describe('Admin proof verification decision endpoints', () => {
    ...
    it('observer admin cannot approve a proof (403)', async () => {
        ...
        expect(response.json()).toMatchObject({ error: 'observer_cannot_write' });
    });
    it('observer admin cannot reject a proof (403)', async () => {
        const ug = await insertCountry('UG', 'Uganda');
        const observer = await insertUser({
    ...
    it('super admin approve sets correct DB fields', async () => {
        ...
        await pool.query(`INSERT INTO escrow_ledger (campaign_id, amount_total, amount_available, currency, status)
       VALUES ($1, 2500, 2500, 'UGX', 'FUNDED')`, [campaign.id]);
        await pool.query(`INSERT INTO contracts (campaign_id, ambassador_id, status)
       VALUES ($1, $2, 'ACTIVE')`, [campaign.id, ambassador.id]);
