...
async function ensureWalletForVendorUser(client, userId) {
    const existing = await client.query(`SELECT * FROM wallets WHERE user_id=$1 LIMIT 1`, [userId]);
    if (existing.rows[0])
        return existing.rows[0];
    const created = await client.query(`INSERT INTO wallets (user_id, currency, balance, balance_available, balance_escrow)
     VALUES ($1, 'UGX', 0, 0, 0)
     RETURNING *`, [userId]);
    return created.rows[0];
}
async function debitVendorWallet(client, input) {
    ...
    app.post('/vendor/boosts', {
        ...
        try {
            ...
            const result = await withTransaction(async (client) => {
                if (listing_id) {
                    const listingRows = await client.query(`SELECT id FROM advert_listings
             WHERE id = $1 AND vendor_id = $2 AND status = 'ACTIVE'
             LIMIT 1`, [listing_id, vendor.id]);
                    if (!listingRows.rows[0])
                ...
                if ('error' in debit)
                    return debit;
                const rows = await client.query(`INSERT INTO vendor_boosts (id, vendor_id, listing_id, plan_id, boost_type, ends_at, cost_ugx)
           VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`, [boostId, vendor.id, listing_id ?? null, plan_id, plan.boost_type, endsAt, amount]);
                return { ok: true, boost: rows.rows[0] };
            });
            if ('error' in result) {
