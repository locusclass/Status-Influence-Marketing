...
async function ensureActiveListingType(client, listingTypeId) {
...
export async function advertRoutes(app) {
    ...
    app.get('/admin/advert/listings/:slug', {
        ...
        `, [slug]);
                if (!row.rows[0])
                    throw Object.assign(new Error('listing_not_found'), { statusCode: 404 });
                const media = await client.query(`SELECT id, media_pack, media_type, url, thumbnail_url, file_name, mime_type, file_size_bytes, sort_order, created_at
           FROM advert_media WHERE listing_id = $1 ORDER BY media_pack, sort_order`, [row.rows[0].id]);
    ...
    app.get('/admin/advert/listings/:slug/analytics', {
        ...
        try {
            const result = await withTransaction(async (client) => {
                const listingRow = await client.query(`SELECT id, title, views_total, views_unique FROM advert_listings WHERE slug = $1`, [slug]);
                if (!listingRow.rows[0])
                    throw Object.assign(new Error('listing_not_found'), { statusCode: 404 });
                const listingId = listingRow.rows[0].id;
        ...
        try {
            const result = await withTransaction(async (client) => {
                ...
                if (body.admin_note !== undefined) {
                    fields.push(`admin_note = $${vals.length + 1}`);
                    vals.push(body.admin_note);
                }
                if (!fields.length)
                    throw Object.assign(new Error('no_fields'), { statusCode: 400 });
                fields.push(`updated_at = now()`);
                const row = await client.query(`UPDATE advert_offers SET ${fields.join(', ')} WHERE id = $1 RETURNING *`, vals);
                if (!row.rows[0])
                    throw Object.assign(new Error('offer_not_found'), { statusCode: 404 });
    ...
    app.post('/admin/advert/categories', {
        ...
        try {
            ...
            const result = await withTransaction(async (client) => {
                const row = await client.query(`INSERT INTO advert_categories (name, slug, icon, description) VALUES ($1, $2, $3, $4) RETURNING *`, [body.name, slug, body.icon ?? null, body.description ?? null]);
    ...
    app.patch('/admin/advert/categories/:categoryId', {
        ...
        try {
            const result = await withTransaction(async (client) => {
                const fields = [];
                const vals = [categoryId];
                if (body.name) {
                    fields.push(`name = $${vals.length + 1}`);
                    vals.push(body.name);
                }
                if (body.icon !== undefined) {
                    fields.push(`icon = $${vals.length + 1}`);
                    vals.push(body.icon);
                }
                if (body.description !== undefined) {
                ...
                const row = await client.query(`UPDATE advert_categories SET ${fields.join(', ')} WHERE id = $1 RETURNING *`, vals);
                if (!row.rows[0])
    ...
    app.post('/admin/advert/subcategories', {
        ...
        try {
            const slug = body.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
            const result = await withTransaction(async (client) => {
                const row = await client.query(`INSERT INTO advert_subcategories (category_id, name, slug) VALUES ($1, $2, $3) RETURNING *`, [body.category_id, body.name, slug]);
            ...
            if (String(err.code) === '23505')
                return reply.code(409).send({ error: 'subcategory_exists' });
            return reply.code(500).send({ error: 'internal_server_error' });
        }
    });
    app.patch('/admin/advert/subcategories/:subcategoryId', {
        ...
        try {
            const result = await withTransaction(async (client) => {
                const row = await client.query(`UPDATE advert_subcategories SET name = COALESCE($2, name) WHERE id = $1 RETURNING *`, [subcategoryId, body.name ?? null]);
                if (!row.rows[0])
                    throw Object.assign(new Error('not_found'), { statusCode: 404 });
                return { subcategory: row.rows[0] };
            });
    ...
    app.post('/admin/advert/listing-types', {
        ...
        try {
            const slug = body.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
            const result = await withTransaction(async (client) => {
                const row = await client.query(`INSERT INTO advert_listing_types (subcategory_id, name, slug) VALUES ($1, $2, $3) RETURNING *`, [body.subcategory_id, body.name, slug]);
            ...
            if (String(err.code) === '23505')
                return reply.code(409).send({ error: 'type_exists' });
            return reply.code(500).send({ error: 'internal_server_error' });
        }
    });
    app.patch('/admin/advert/listing-types/:typeId', {
        ...
        try {
            const result = await withTransaction(async (client) => {
                const row = await client.query(`UPDATE advert_listing_types SET name = COALESCE($2, name) WHERE id = $1 RETURNING *`, [typeId, body.name ?? null]);
                if (!row.rows[0])
                    throw Object.assign(new Error('not_found'), { statusCode: 404 });
                return { listing_type: row.rows[0] };
            });
    ...
    app.post('/admin/advert/field-definitions', {
        ...
        try {
            const result = await withTransaction(async (client) => {
                const row = await client.query(`INSERT INTO advert_field_definitions (listing_type_id, field_key, label, field_type, is_required, sort_order, options_json)
           VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`, [body.listing_type_id, body.field_key, body.label, body.field_type, body.is_required, body.sort_order, body.options_json ? JSON.stringify(body.options_json) : null]);
            ...
            if (String(err.code) === '23505')
                return reply.code(409).send({ error: 'field_exists' });
            return reply.code(500).send({ error: 'internal_server_error' });
        }
    });
        ...
        try {
            const result = await withTransaction(async (client) => {
                ...
                if (body.is_required !== undefined) {
                    fields.push(`is_required = $${vals.length + 1}`);
                    vals.push(body.is_required);
                }
                if (body.sort_order !== undefined) {
                    fields.push(`sort_order = $${vals.length + 1}`);
                    vals.push(body.sort_order);
                }
                if (body.options_json !== undefined) {
                    fields.push(`options_json = $${vals.length + 1}`);
                    vals.push(body.options_json ? JSON.stringify(body.options_json) : null);
                }
                if (!fields.length)
                    return reply.code(400).send({ error: 'no_fields' });
                const row = await client.query(`UPDATE advert_field_definitions SET ${fields.join(', ')} WHERE id = $1 RETURNING *`, vals);
                if (!row.rows[0])
                    throw Object.assign(new Error('not_found'), { statusCode: 404 });
                return { field_definition: row.rows[0] };
            });
