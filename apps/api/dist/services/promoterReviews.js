export async function ensurePromoterReviewsSchema(client) {
    await client.query(`
...
export async function loadPromoterReviewSummaryMap(client, promoterIds) {
    const ids = Array.from(new Set(promoterIds
...
export async function listPromoterReviews(client, promoterId, limit = 12) {
    const res = await client.query(`
    SELECT
      review.id,
      review.promoter_id,
...
export async function loadLatestCompletedContractForReview(client, advertiserId, promoterId) {
    const res = await client.query(`
    SELECT
      ctr.id,
      ctr.status,
