...
export async function ensurePrimeRequestsSchema(client) {
    if (schemaEnsured)
        return;
    await client.query(`
...
export function normalizePrimeRequestStatus(value) {
...
export function normalizePrimeRequestPassType(value) {
    const normalized = String(value ?? '').trim().toLowerCase();
    return normalized in PRIME_REQUEST_ACCESS_PASS_PLANS
        ? normalized
        : null;
}
export function publicPrimeRequestDto(row) {
...
export function fullPrimeRequestDto(row) {
    return {
...
export function adminPrimeRequestDto(row) {
    return {
...
export async function getActivePrimeRequestPass(client, userId) {
...
export async function hasPrimeRequestAccess(client, requestId, userId) {
    await ensurePrimeRequestsSchema(client);
    const res = await client.query(`SELECT EXISTS (
     ...
     ) AS has_pass`, [requestId, userId]);
    const row = res.rows[0] ?? {};
    return row.is_owner === true || row.has_unlock === true || row.has_pass === true;
}
export async function loadPrimeRequest(client, requestId) {
    await ensurePrimeRequestsSchema(client);
    const res = await client.query(`SELECT * FROM prime_requests WHERE id=$1 LIMIT 1`, [requestId]);
    return res.rows[0] ?? null;
}
export async function insertPrimeRequestUnlock(client, input) {
    await ensurePrimeRequestsSchema(client);
    const id = uuid();
    const res = await client.query(`INSERT INTO prime_request_unlocks
...
export async function insertPrimeRequestAccessPass(client, input) {
    await ensurePrimeRequestsSchema(client);
    const plan = PRIME_REQUEST_ACCESS_PASS_PLANS[input.passType];
    const id = uuid();
    const res = await client.query(`INSERT INTO prime_request_access_passes
...
export async function ensureWalletForPrimeRequestUser(client, userId) {
    const existing = await client.query(`SELECT * FROM wallets WHERE user_id=$1 LIMIT 1`, [userId]);
    if (existing.rows[0])
        return existing.rows[0];
    const inserted = await client.query(`INSERT INTO wallets (user_id, currency, balance, balance_available, balance_escrow)
     VALUES ($1, 'UGX', 0, 0, 0)
     RETURNING *`, [userId]);
    return inserted.rows[0];
}
export async function debitPrimeRequestWallet(client, input) {
    ...
    await client.query(`UPDATE wallets
     ...
     WHERE id = $1`, [wallet.id, input.amount]);
    await client.query(`INSERT INTO wallet_txns (wallet_id, amount, direction, reference)
     VALUES ($1, $2, 'DEBIT', $3)`, [wallet.id, input.amount, input.reference]);
    return { ok: true, wallet_id: wallet.id };
}
export function normalizePrimeRequestCreateInput(body) {
    return {
...
export function validatePrimeRequestCreateInput(input) {
    const issues = [];
    if (input.title.length < 5 || input.title.length > 160)
        issues.push({ path: ['title'], message: 'Title must be 5-160 characters.' });
    if (input.description.length < 20 || input.description.length > 5000)
        issues.push({ path: ['description'], message: 'Description must be 20-5000 characters.' });
    if (input.category.length < 2 || input.category.length > 80)
        issues.push({ path: ['category'], message: 'Category is required.' });
    if (input.location.length < 2 || input.location.length > 160)
        issues.push({ path: ['location'], message: 'Location is required.' });
    if (input.urgency.length < 2 || input.urgency.length > 80)
        issues.push({ path: ['urgency'], message: 'Urgency is required.' });
    if (input.requester_name.length < 2 || input.requester_name.length > 120)
        issues.push({ path: ['requester_name'], message: 'Name is required.' });
    if (input.requester_phone.length < 7 || input.requester_phone.length > 30)
        issues.push({ path: ['requester_phone'], message: 'Enter a valid phone number.' });
    if (input.requester_whatsapp && input.requester_whatsapp.length > 30)
        issues.push({ path: ['requester_whatsapp'], message: 'Enter a valid WhatsApp number.' });
    if (!input.consent_provider_contact)
        issues.push({ path: ['consent_provider_contact'], message: 'Consent is required.' });
    if (input.budget_min != null && input.budget_min < 0)
        issues.push({ path: ['budget_min'], message: 'Budget minimum cannot be negative.' });
    if (input.budget_max != null && input.budget_max < 0)
        issues.push({ path: ['budget_max'], message: 'Budget maximum cannot be negative.' });
...
export async function expireOldPrimeRequests(client) {
    await ensurePrimeRequestsSchema(client);
    await client.query(`UPDATE prime_requests
     SET status='expired', updated_at=now()
     WHERE expires_at IS NOT NULL
       AND expires_at <= now()
       AND status IN ('approved', 'active')`);
    await client.query(`UPDATE prime_request_access_passes
     SET status='expired', updated_at=now()
