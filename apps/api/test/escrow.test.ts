...
describe('Escrow idempotency', () => {
  ...
  it('prevents double payout using unique proof_id', async () => {
    const user = await pool.query("INSERT INTO users (email, phone, password_hash, role) VALUES ('a@b.com','+254700000001','x','AMBASSADOR') RETURNING *");
    const campaign = await pool.query("INSERT INTO campaigns (business_id, title, platform, payout_amount, budget_total, start_date, end_date) VALUES ($1,'t','WHATSAPP_STATUS',100,1000,'2024-01-01','2024-12-31') RETURNING *", [user.rows[0].id]);
