...
function counterpartyName(row: any) {
  return String(
...
function serializeThreadSummary(row: any): ChatThreadSummary {
  ...
  return {
    ...
    last_activity_at: timestampText(
      row.last_activity_at ?? row.last_message_at ?? row.created_at
    ),
...
export async function chatRoutes(app: FastifyInstance) {
  ...
  app.post('/chat/threads/:id/messages', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    const result = await withTransaction(async (client) => {
      ...
      const senderName = String(
        senderNameRes.rows[0]?.sender_name ?? 'New message'
      ).trim();
  ...
  app.post('/chat/threads/:id/typing', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    const isTyping =
      parsed.data.is_typing ?? draftText.length > 0;
