...
async function ensureUniqueVendorSlug(base: string): Promise<string> {
  let slug = base;
  let attempt = 0;
  while (attempt < 10) {
    const rows = await query<any>(
      `SELECT id FROM vendor_profiles WHERE slug = $1 LIMIT 1`,
      [slug]
    );
...
async function getVendorForUser(userId: string) {
  const rows = await query<any>(
    `SELECT * FROM vendor_profiles WHERE user_id = $1 LIMIT 1`,
    [userId]
  );
  return rows[0] ?? null;
}
...
export async function vendorRoutes(app: FastifyInstance) {
  ...
  app.get('/vendor/profile', {
    ...
    try {
      ...
      const listingCount = await query<any>(
        `SELECT COUNT(*)::int AS total FROM advert_listings WHERE vendor_id = $1 AND status = 'ACTIVE'`,
        [vendor.id]
      );
  ...
  app.patch('/vendor/profile', {
    ...
    try {
      ...
      if (body.vendor_name !== undefined)       add('vendor_name',       body.vendor_name);
      if (body.description !== undefined)       add('description',       body.description ?? null);
      if (body.phone_number !== undefined)      add('phone_number',      body.phone_number ?? null);
      if (body.whatsapp_number !== undefined)   add('whatsapp_number',   body.whatsapp_number ?? null);
      if (body.email !== undefined)             add('email',             body.email ?? null);
      if (body.business_location !== undefined) add('business_location', body.business_location ?? null);
      if (body.business_category !== undefined) add('business_category', body.business_category ?? null);
      if (body.logo_url !== undefined)          add('logo_url',          body.logo_url ?? null);
      if (body.banner_url !== undefined)        add('banner_url',        body.banner_url ?? null);
      if (body.social_instagram !== undefined)  add('social_instagram',  body.social_instagram ?? null);
      if (body.social_facebook !== undefined)   add('social_facebook',   body.social_facebook ?? null);
      if (body.social_twitter !== undefined)    add('social_twitter',    body.social_twitter ?? null);
      if (body.social_tiktok !== undefined)     add('social_tiktok',     body.social_tiktok ?? null);
      if (body.website_url !== undefined)       add('website_url',       body.website_url ?? null);

      if (sets.length === 0) return { ok: true, vendor };

      sets.push(`updated_at = NOW()`);
      vals.push(vendor.id);
      const updated = await query<any>(
        `UPDATE vendor_profiles SET ${sets.join(', ')} WHERE id = $${vals.length} RETURNING *`,
        vals as any[]
      );
  ...
  app.get('/vendor/inquiries', {
    ...
    try {
      ...
      const [rows, countRows] = await Promise.all([
        ...
        query<any>(`SELECT COUNT(*)::int AS total FROM vendor_inquiries vi WHERE ${where}`, params),
      ]);

      // Mark fetched inquiries as READ if they were NEW
      const newIds = (rows as any[]).filter(r => r.status === 'NEW').map(r => r.id);
      if (newIds.length > 0) {
        await query(
          `UPDATE vendor_inquiries SET status = 'READ', updated_at = NOW()
           WHERE id = ANY($1::uuid[])`,
          [newIds]
        );
      }
  ...
  app.post('/vendor/inquiries/:id/reply', {
    ...
    try {
      ...
      const inquiryRows = await query<any>(
        `SELECT id, vendor_id FROM vendor_inquiries WHERE id = $1 LIMIT 1`, [id]
      );
      const inquiry = inquiryRows[0];
      if (!inquiry) return reply.code(404).send({ error: 'inquiry_not_found' });
      if (inquiry.vendor_id !== vendor.id) return reply.code(403).send({ error: 'forbidden' });

      await query(
        `UPDATE vendor_inquiries
         SET reply = $1, replied_at = NOW(), status = 'REPLIED', updated_at = NOW()
         WHERE id = $2`,
        [parsed.data.reply, id]
      );

      // bump counter
      await query(
        `UPDATE vendor_profiles SET total_inquiries = total_inquiries + 1, updated_at = NOW()
  ...
  app.get('/vendor/negotiations', {
    ...
    try {
      ...
      if (status) { params.push(status); conditions.push(`nt.status = $${params.length}`); }
      ...
      const [rows, countRows] = await Promise.all([
        ...
        query<any>(`SELECT COUNT(*)::int AS total FROM negotiation_threads nt WHERE ${where}`, params),
      ]);
  ...
  app.get('/vendor/negotiations/:id', {
    ...
    try {
      ...
      const threadRows = await query<any>(
        `SELECT nt.*, al.title AS listing_title, al.slug AS listing_slug, al.price AS listing_price
         FROM negotiation_threads nt
         JOIN advert_listings al ON al.id = nt.listing_id
         WHERE nt.id = $1 AND nt.vendor_id = $2 LIMIT 1`,
        [id, vendor.id]
      );
      ...
      const messages = await query<any>(
        `SELECT * FROM negotiation_messages WHERE thread_id = $1 ORDER BY created_at ASC`, [id]
      );

      // Mark buyer messages as read
      await query(
        `UPDATE negotiation_messages SET is_read = TRUE
  ...
  app.post('/vendor/negotiations/:id/respond', {
    ...
    try {
      ...
      const threadRows = await query<any>(
        `SELECT * FROM negotiation_threads WHERE id = $1 AND vendor_id = $2 LIMIT 1`,
        [id, vendor.id]
      );
      const thread = threadRows[0];
      if (!thread) return reply.code(404).send({ error: 'thread_not_found' });
      ...
      await withTransaction(async (client) => {
        ...
        const content = message ?? (
          action === 'ACCEPT' ? 'I accept your offer.' :
          action === 'REJECT' ? 'Sorry, I cannot proceed with this offer.' :
          `Counter offer: ${thread.currency} ${offer_amount?.toLocaleString()}`
        );

        await client.query(
          `INSERT INTO negotiation_messages (id, thread_id, party, message_type, content, offer_amount, currency)
           VALUES ($1, $2, 'VENDOR', $3, $4, $5, $6)`,
          [uuid(), id, messageType, content, offer_amount ?? null, thread.currency]
        );
        ...
        await client.query(
          `UPDATE negotiation_threads
           ...
           WHERE id = $3`,
          [newStatus, offer_amount ?? null, id]
        );

        if (action === 'ACCEPT') {
          await client.query(
            `UPDATE vendor_profiles SET total_negotiations = total_negotiations + 1, updated_at = NOW()
             WHERE id = $1`, [vendor.id]
          );
        }
      });
  ...
  app.get('/vendor/boosts', {
    ...
    try {
      ...
      const [boosts, plans] = await Promise.all([
        ...
        query<any>(`SELECT * FROM vendor_boost_plans WHERE is_active = TRUE ORDER BY sort_order ASC`),
      ]);
  ...
  app.post('/vendor/boosts', {
    ...
    try {
      ...
      const planRows = await query<any>(
        `SELECT * FROM vendor_boost_plans WHERE id = $1 AND is_active = TRUE LIMIT 1`, [plan_id]
      );
  ...
  app.post('/vendor/analytics/event', async (request, reply) => {
    ...
    try {
      ...
      if (event_type === 'STOREFRONT_VIEW') {
        await query(
          `UPDATE vendor_profiles SET total_views = total_views + 1 WHERE id = $1`, [vendor_id]
        );
      } else if (event_type === 'QR_SCAN') {
        await query(
          `UPDATE vendor_profiles SET qr_scan_count = qr_scan_count + 1 WHERE id = $1`, [vendor_id]
        );
        if (listing_id) {
          await query(
            `UPDATE advert_listings SET qr_scan_count = qr_scan_count + 1 WHERE id = $1`, [listing_id]
          );
        }
      } else if (event_type === 'LISTING_VIEW' && listing_id) {
        await query(
          `UPDATE advert_listings SET views_total = COALESCE(views_total, 0) + 1 WHERE id = $1`,
          [listing_id]
        );
      }
