...
import {
  ...
  claimAdminOperationTask,
  createAdminOperationMessage,
  ensureAdminOperationsSchema,
  loadAdminOperationTaskState,
  loadAdminOperationsSnapshot,
  loadVisibleAdminOperationTask,
...
const AdminSettingsPatchSchema = z
  ...
  .refine(
    (value) =>
      value.campaign_approval_mode !== undefined ||
...
async function markContractCompletedForVerifiedProof(client: any, proofId: string) {
  ...
  const escrowRes = await client.query(
    ...
    [proofContext.escrow_campaign_id]
  );
...
async function ensureContractParticipantColumns(client: any) {
  await client.query(`
...
async function ensureProofReviewColumns(client: any) {
  await client.query(`
    ALTER TABLE proofs
...
export async function adminRoutes(app: FastifyInstance) {
  ...
  app.get('/admin/verification-sessions', { preHandler: [app.adminOnly] }, async (request) => {
    ...
    return withTransaction(async (client) => {
      ...
      if (query?.state) {
        conditions.push(
          `(CASE WHEN p.id IS NOT NULL THEN 'PROOF_SUBMITTED' WHEN s.expires_at < now() THEN 'EXPIRED' ELSE 'ACTIVE' END) = $${idx}`
        );
  ...
  app.get('/admin/users', { preHandler: [app.adminOnly] }, async (request) => {
    ...
    return withTransaction(async (client) => {
      ...
      if (query?.q) {
        conditions.push(
          `(u.email ILIKE $${idx} OR COALESCE(u.full_name, '') ILIKE $${idx} OR u.phone ILIKE $${idx} OR u.id::text ILIKE $${idx} OR u.public_id ILIKE $${idx} OR COALESCE(u.country, '') ILIKE $${idx})`
        );
        params.push(`%${query.q}%`);
        idx++;
      }
  ...
  app.get('/admin/users/:id/detail', { preHandler: [app.adminOnly] }, async (request, reply) => {
    ...
    const result = await withTransaction(async (client) => {
      ...
      const statusSummaries = await buildCampaignStatusSummaries(
        client,
        campaignsRes.rows.map((row: any) => String(row.id)),
        null
      );
      ...
      return {
        ...
        campaigns: campaignsRes.rows.map((row: any) => ({
          ...row,
  ...
  app.get('/admin/campaigns', { preHandler: [app.adminOnly] }, async (request) => {
    ...
    return withTransaction(async (client) => {
      ...
      if (query?.q) {
        conditions.push(
          `(c.title ILIKE $${idx} OR c.id::text ILIKE $${idx} OR c.public_id ILIKE $${idx} OR adv.email ILIKE $${idx} OR adv.public_id ILIKE $${idx} OR c.business_id::text ILIKE $${idx} OR dist.email ILIKE $${idx} OR dist.public_id ILIKE $${idx} OR COALESCE(c.assigned_ambassador_id::text, '') ILIKE $${idx})`
        );
        params.push(`%${query.q}%`);
        idx++;
      }
      ...
      const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
      const res = await client.query(
        `SELECT c.*, adv.email AS business_email, adv.public_id AS business_public_id, dist.email AS assigned_ambassador_email, dist.public_id AS assigned_ambassador_public_id FROM campaigns c LEFT JOIN users adv ON adv.id = c.business_id LEFT JOIN users dist ON dist.id = c.assigned_ambassador_id ${where} ORDER BY c.created_at DESC LIMIT $${idx} OFFSET $${idx + 1}`,
        [...params, limit, offset]
      );
      const statusSummaries = await buildCampaignStatusSummaries(
        client,
    ...
    const res = await withTransaction(async (client) => {
      ...
      const updated = await client.query(
        `UPDATE proofs
         ...
         WHERE id=$1
         RETURNING *`,
        [params.id, body.status ?? null, body.decision ?? null, body.observed_views ?? null]
      );
      if (updated.rows[0]) {
        await logAudit(
          client,
  ...
  app.patch('/admin/user-verifications/:id/approve', { preHandler: [app.adminOnly] }, async (request, reply) => {
    ...
    return withTransaction(async (client) => {
      ...
      await client.query(
        `UPDATE users SET max_status_viewers_12h=$2 WHERE id=$1`,
        [rec.resolved_user_id, body.viewer_count]
      );
  ...
  app.post('/admin/operations/tasks/:taskKey/claim', { preHandler: [app.adminOnly] }, async (request, reply) => {
    ...
    return withTransaction(async (client) => {
      ...
      const slaMinutes = Math.min(
        Math.max(Number(snapshot.settings.operations_sla_minutes ?? 10), 1),
        120
      );
      const defaultAckMinutes = Math.min(
        Math.max(Number(snapshot.settings.operations_ack_minutes ?? 5), 1),
        slaMinutes
      );
      const ackMinutes = Math.min(
        Math.max(body.data.minutes ?? defaultAckMinutes, 1),
        slaMinutes
      );
  ...
  app.post('/admin/operations/tasks/:taskKey/release', { preHandler: [app.adminOnly] }, async (request, reply) => {
    ...
    return withTransaction(async (client) => {
      ...
      if (
        currentState.claimed_by_user_id &&
    ...
    return withTransaction(async (client) => {
      ...
      if (body.data.campaign_approval_mode !== undefined) {
        await client.query(
          ...
          [body.data.campaign_approval_mode]
        );
      }
      if (body.data.operations_sla_minutes !== undefined) {
        await client.query(
          `INSERT INTO admin_settings (key, value, updated_at)
           ...
           ON CONFLICT (key) DO UPDATE SET value=$1, updated_at=now()`,
          [String(body.data.operations_sla_minutes)]
        );
      }
      if (body.data.operations_ack_minutes !== undefined) {
        const maxAckMinutes = body.data.operations_sla_minutes ?? Number((
          await client.query(
            `SELECT value FROM admin_settings WHERE key = 'operations_sla_minutes' LIMIT 1`
          )
        ).rows[0]?.value ?? 10);
        ...
        await client.query(
          `INSERT INTO admin_settings (key, value, updated_at)
           ...
           ON CONFLICT (key) DO UPDATE SET value=$1, updated_at=now()`,
          [String(body.data.operations_ack_minutes)]
        );
      }
  ...
  app.patch('/admin/campaign-completions/:proofId/approve', { preHandler: [app.adminOnly] }, async (request, reply) => {
    ...
    return withTransaction(async (client) => {
      ...
      await client.query(
        `UPDATE proofs SET status='VERIFIED', decision='VERIFIED', updated_at=now() WHERE id=$1`,
        [params.proofId]
      );
  ...
  app.patch('/admin/campaign-completions/:proofId/reject', { preHandler: [app.adminOnly] }, async (request, reply) => {
    ...
    return withTransaction(async (client) => {
      ...
      await client.query(
        `UPDATE proofs SET status='REJECTED', decision='REJECTED', updated_at=now() WHERE id=$1`,
        [params.proofId]
      );
  ...
  app.post('/admin/campaigns/:id/system-fund', { preHandler: [app.adminOnly] }, async (request, reply) => {
    ...
    const result = await withTransaction(async (client) => {
      ...
      if (!escrowId) {
        ...
        await client.query(
          'UPDATE escrow_ledger SET amount_total = $2, amount_available = $2 WHERE id = $1',
          [escrowId, body.amount]
        );
      ...
      const txnRes = await client.query(
        `INSERT INTO pesapal_transactions (escrow_id, type, amount, merchant_reference, status, raw_payload)
  ...
  app.get('/admin/contracts', { preHandler: [app.adminOnly] }, async (request) => {
    ...
    return withTransaction(async (client) => {
      ...
      const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
      const res = await client.query(
        `SELECT ctr.*, c.title AS campaign_title, u.email AS ambassador_email, adv.email AS business_email FROM contracts ctr JOIN campaigns c ON c.id = ctr.campaign_id JOIN users u ON u.id = ctr.ambassador_id JOIN users adv ON adv.id = c.business_id
         ${where}
         ORDER BY ctr.created_at DESC
         LIMIT $${idx} OFFSET $${idx + 1}`,
        [...params, limit, offset]
  ...
  app.patch('/admin/contracts/:id', { preHandler: [app.adminOnly] }, async (request, reply) => {
    ...
    const res = await withTransaction(async (client) => {
      ...
      if (body.ambassador_id) {
        ...
        if (!ambassador) {
          reply.code(404);
          return { error: 'ambassador_not_found' } as any;
        }
        const sameTenant =
          String(ambassador.country_id ?? '') ===
        ...
        if (!sameTenant || !matchesTenantScope(access, ambassador)) {
          reply.code(409);
          return { error: 'ambassador_scope_mismatch' } as any;
        }
      }
      const updated = await client.query(
        `UPDATE contracts
         ...
         WHERE id=$1
         RETURNING *`,
        [params.id, body.status ?? null, body.ambassador_id ?? null]
      );
      if (updated.rows[0]) {
        await logAudit(
          client,
    ...
    if (
      (res as any)?.error === 'ambassador_not_found' ||
      (res as any)?.error === 'ambassador_scope_mismatch'
    ) {
      return res;
    }
    if (!res) {
