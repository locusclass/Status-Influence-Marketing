...
async function fetchSystemContext(client: any): Promise<string> {
  const now = new Date().toISOString();

  const [jobStats, proofStats, campaignStats, failedJobs, payoutStats, ambassadorCount, recentProofs] =
    await Promise.all([
      client.query(`
  ...
  const lines: string[] = [
    ...
    ...(jobStats.rows.length
      ? jobStats.rows.map((r: any) => `  ${r.job_type} | ${r.status}: ${r.count}`)
    ...
    ...(proofStats.rows.length
      ? proofStats.rows.map(
          (r: any) =>
            `  status=${r.status} verification=${r.verification_status ?? 'N/A'}: ${r.count}`
        )
      : ['  No proofs found']),
    '',
    '## CAMPAIGNS',
    ...(campaignStats.rows.length
      ? campaignStats.rows.map((r: any) => `  ${r.status}: ${r.count}`)
      : ['  No campaigns found']),
    '',
    '## PAYOUTS (last 7 days)',
    ...(payoutStats.rows.length
      ? payoutStats.rows.map((r: any) => `  ${r.status}: ${r.count} payouts, total UGX ${r.total}`)
    ...
    ...(recentProofs.rows.length
      ? recentProofs.rows.map(
          (r: any) =>
            `  [${String(r.id).slice(0, 8)}] "${r.campaign_title ?? 'unknown'}" status=${r.status} verification=${r.verification_status ?? 'N/A'} ai_rec=${r.ai_recommendation ?? 'N/A'} fraud=${r.fraud_risk_score ?? 'N/A'} views=${r.viewer_count_detected ?? 'N/A'} at=${r.created_at}`
        )
      : ['  No proofs yet']),
    '',
    '## RECENT FAILED JOBS',
    ...(failedJobs.rows.length
      ? failedJobs.rows.map(
          (r: any) => `  ${r.job_type} at ${r.created_at}: ${r.error ?? 'no error message'}`
        )
      : ['  None']),
  ];
...
export async function aiAdminRoutes(app: FastifyInstance) {
  app.post(
    ...
    async (request, reply) => {
      const access = await withTransaction((client) =>
