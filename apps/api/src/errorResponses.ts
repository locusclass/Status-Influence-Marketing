...
const EXPLICIT_USER_MESSAGES: Record<string, string> = {
  unauthorized: 'Please sign in to continue.',
  forbidden: 'You do not have permission to do that.',
  admin_suspended: 'This admin account is suspended.',
  policy_acceptance_required:
    'You need to accept the required policies before continuing.',
  ...
  missing_signature:
    'This upload link is incomplete. Please request a new one.',
  ...
  yo_uganda_not_configured:
    'Payments are temporarily unavailable. Please try again later.',
  yo_uganda_initiate_failed:
    'We could not start the payment right now. Please try again.',
  yo_uganda_verify_failed:
  ...
  firebase_client_init_failed:
    'File upload is temporarily unavailable. Please try again.',
  firebase_storage_upload_failed:
    'File upload is temporarily unavailable. Please try again.',
  firebase_storage_download_failed:
    'File download is temporarily unavailable. Please try again.',
  firebase_storage_delete_failed:
    'File deletion is temporarily unavailable. Please try again.',
  ...
  unsupported_payment_method:
    'This payment method is not available for this transaction.',
...
function looksTechnicalText(value: string) {
  ...
  if (isCodeLike(text)) {
    return /^(?:fst_err_|und_err_|econn|enotfound|ehostunreach|fatal_startup_configuration|pg_|sqlstate_|\d{5}$)/i.test(
      text
    );
  }

  return (
    /\b(?:syntaxerror|typeerror|referenceerror|postgres|sql|query failed|duplicate key|constraint|timeout|timed out|stack|trace|unexpected token|unexpected end of json input|invalid json|request failed|network error|fetch failed|connectionlosterror|fasterxml|firebase_|yo uganda request failed|und_err_|econn|enotfound|ehostunreach|fatal_startup_configuration)\b/i.test(
      text
    ) ||
    /https?:\/\//i.test(text) ||
...
function baseCode(value: string) {
  if (!isCodeLike(value)) return value.trim();
  return value.split(':')[0]!.trim().toLowerCase();
}

function classifyTechnicalError(value: string, statusCode: number) {
  const text = value.trim().toLowerCase();

  if (
    text.startsWith('fst_err_') ||
...
export function normalizePublicErrorCode(value: unknown, statusCode: number) {
...
function resolveMessageFromCode(code: string, statusCode: number) {
  if (!code) {
  ...
  if (normalized.startsWith('missing_')) {
    return toSentence(
      `${normalized.slice('missing_'.length).replace(/_/g, ' ')} is required`
    );
  }
  if (normalized.endsWith('_required')) {
    return toSentence(
      `${normalized.slice(0, -'_required'.length).replace(/_/g, ' ')} is required`
    );
  }
  if (normalized.startsWith('invalid_')) {
    return toSentence(
      `The provided ${normalized.slice('invalid_'.length).replace(/_/g, ' ')} is invalid`
    );
  }
  if (
    normalized.endsWith('_taken') ||
...
function shouldKeepDetail(
  ...
  if (looksTechnicalText(detail)) return false;
  return detail.trim().length > 0;
}

function shouldKeepDetails(
  code: string,
  statusCode: number,
  details: unknown
) {
  ...
  if (statusCode >= 500) return false;
  if (Array.isArray(details) || isRecord(details)) return true;
  if (typeof details !== 'string') return false;
  return !looksTechnicalText(details);
}

export function maskErrorResponsePayload(payload: unknown, statusCode: number) {
  if (statusCode < 400 || !isRecord(payload)) {
    return payload;
  }

  const result: ErrorPayload = { ...payload };
  const publicError = normalizePublicErrorCode(result.error, statusCode);
  const shouldExposeCode = isCodeLike(publicError);

  const currentMessage =
    typeof result.message === 'string' && !looksTechnicalText(result.message)
      ? result.message.trim()
      : '';
