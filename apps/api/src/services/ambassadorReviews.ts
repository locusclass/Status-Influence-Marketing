export type AmbassadorReviewSummary = {
  average_rating: number;
  rating_count: number;
  latest_comment: string | null;
};

export async function ensureAmbassadorReviewsSchema(client: any) {
  await client.query(`
...
export async function loadAmbassadorReviewSummaryMap(
  client: any,
  ...
  if (ids.length === 0) {
    return new Map<string, AmbassadorReviewSummary>();
  }
  ...
  const map = new Map<string, AmbassadorReviewSummary>();
...
export async function listAmbassadorReviews(
  client: any,
  ambassadorId: string,
  limit = 12
) {
