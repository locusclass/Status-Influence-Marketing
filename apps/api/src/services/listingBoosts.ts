...
export const LISTING_BOOST_PLANS = {
  FEATURED_24H: {
    ...
    description:
      'Pushes your product page ahead of standard listings for one full day.',
    duration_hours: 24,
    amount_ugx: 5_000,
  },
  FEATURED_72H: {
    ...
    description:
      'Keeps your product page prioritized across the marketplace for three days.',
    duration_hours: 72,
    amount_ugx: 12_000,
  },
  FEATURED_168H: {
    ...
    description:
      'Extends priority exposure for an entire week while your campaign remains live.',
    duration_hours: 168,
    amount_ugx: 25_000,
  },
} as const;

export type ListingBoostPlanCode = keyof typeof LISTING_BOOST_PLANS;
...
export function resolveListingBoostPlan(code: string) {
  const normalized = code.trim().toUpperCase() as ListingBoostPlanCode;
  return LISTING_BOOST_PLANS[normalized] ?? null;
}

export function calculateListingBoostPrice(
  planCode: string,
  ...
  const discountPercent = proSubscriptionActive
    ...
    : 0;
  const amountUgx = Math.max(
    0,
    Math.round((plan.amount_ugx * (100 - discountPercent)) / 100)
  );
  return {
    ...plan,
...
export function buildListingBoostOptions(proSubscriptionActive: boolean) {
  return Object.values(LISTING_BOOST_PLANS).map((plan) =>
    calculateListingBoostPrice(plan.code, proSubscriptionActive)
  );
}

export async function ensureListingBoostSchema(client: any) {
  await client.query(`
...
export async function activateListingBoost(
  client: any,
  input: {
    listingId: string;
    businessId: string;
  ...
  const currentBoostExpiry = listing.boost_expires_at
    ...
    : null;
  const boostStartsAt =
    currentBoostExpiry != null && currentBoostExpiry.getTime() > now.getTime()
      ? currentBoostExpiry
      : now;
  let boostEndsAt = new Date(
    boostStartsAt.getTime() + plan.duration_hours * MILLIS_PER_HOUR
  );
  ...
  const nextStatus =
    boostEndsAt.getTime() > now.getTime() ? 'ACTIVE' : 'EXPIRED';
