...
export async function cleanupAdminHandlerJaz(client: any) {
...
export function parseLiveCursor(value: unknown) {
...
export function maxLiveCursor(values: unknown[]) {
...
export async function loadAdminHandlerJazIdentity(client: any, userId: string) {
  await ensureAdminHandlerJazSchema(client);
...
export async function upsertAdminHandlerJazIdentity(
  client: any,
  input: {
    userId: string;
    handle: string;
...
export async function upsertAdminHandlerJazPresence(
  client: any,
  input: {
    userId: string;
    handle: string;
...
export async function deactivateAdminHandlerJazPresence(
  client: any,
  userId: string
) {
  await ensureAdminHandlerJazSchema(client);
...
export async function createAdminHandlerJazMessage(
  client: any,
  input: {
    senderUserId: string;
    senderHandle: string;
...
export async function createAdminHandlerJazSignalEvent(
  client: any,
  input: {
    senderUserId: string;
    senderHandle: string;
  ...
  const ttlMinutes = Math.max(
    1,
    Math.min(Number(input.ttlMinutes ?? ADMIN_HANDLER_JAZ_SIGNAL_TTL_MINUTES), 60)
  );
  const res = await client.query(
    `
...
export async function listAdminHandlerJazParticipants(client: any) {
  await ensureAdminHandlerJazSchema(client);
  const res = await client.query(
    `
    SELECT
...
export async function listAdminHandlerJazMessages(
  client: any,
  options: {
    since?: string | null;
    limit?: number;
...
export async function listAdminHandlerJazSignalEvents(
  client: any,
  userId: string,
  options: {
    since?: string | null;
...
export function serializeAdminHandlerJazParticipant(row: any) {
  return {
...
export function serializeAdminHandlerJazMessage(row: any) {
  return {
    id: String(row?.id ?? ''),
...
export function serializeAdminHandlerJazSignal(row: any) {
  return {
    id: String(row?.id ?? ''),
    sender_user_id: String(row?.sender_user_id ?? ''),
    sender_handle: String(row?.sender_handle ?? ''),
...
function normalizePresenceMode(value: unknown): PresenceMode {
