...
export type AdminOperationTaskRecord = {
  task_key: string;
  entity_type: string;
  entity_id: string;
  category: string;
...
type AdminOperationTaskStateRow = {
  task_key: string;
  entity_type: string;
  entity_id: string;
  category: string;
...
export type AdminOperationTaskInput = {
  task_key: string;
  entity_type: string;
  entity_id: string;
  category: string;
...
export type AdminOperationsSnapshot = {
  ...
  tasks: AdminOperationTaskRecord[];
...
export type AdminOperationsSnapshotOptions = {
...
function compareTasks(a: AdminOperationTaskRecord, b: AdminOperationTaskRecord) {
...
function priorityFor(deadlineAt: string | null) {
  if (!deadlineAt) {
    return 'MONITOR';
  }
...
function summarizeTasks(tasks: AdminOperationTaskRecord[]) {
...
function applyTaskState(
  task: AdminOperationTaskRecord,
  state: AdminOperationTaskStateRow | undefined
) {
  if (!state) {
    return task;
  }
  ...
  const ackActive =
    until != null && Date.parse(until) > Date.now();
...
async function loadAdminSettings(client: PoolClient) {
...
async function loadTaskStates(
  ...
  if (taskKeys.length === 0) {
    return [] as AdminOperationTaskStateRow[];
  }
  const res = await client.query<AdminOperationTaskStateRow>(
    `
    SELECT
      s.task_key,
      s.entity_type,
...
async function loadApprovalTasks(
  ...
  if (!hasAdminModuleAccess(access, ADMIN_MODULE_CAMPAIGNS)) {
    return [] as AdminOperationTaskRecord[];
  }
  const state: FilterState = { conditions: [`c.approval_status = 'PENDING_APPROVAL'`], params: [], idx: 1 };
  ...
  return res.rows.map((row: any) => {
    ...
    const deadlineAt = minTimestamp(
      normalizeTimestamp(row.approval_deadline),
      addMinutes(createdAt, slaMinutes)
    );
    return taskRecordBase({
      ...
      subtitle: normalizeText(
        row.business_name ?? row.business_email,
        'Business review'
      ),
      status: 'PENDING',
...
async function loadCompletionTasks(
  client: PoolClient,
  access: DashboardAccessContext,
  slaMinutes: number
) {
  ...
  return res.rows.map((row: any) => {
    ...
    return taskRecordBase({
      ...
      subtitle: normalizeText(
        row.ambassador_name ?? row.ambassador_email,
        'Ambassador proof'
      ),
...
async function loadRiskTasks(client: PoolClient, access: DashboardAccessContext) {
  if (!hasAdminModuleAccess(access, ADMIN_MODULE_RISK)) {
    return [] as AdminOperationTaskRecord[];
  }

  const trustState: FilterState = { conditions: [], params: [], idx: 1 };
  ...
  const fingerprintState: FilterState = { conditions: [], params: [], idx: 1 };
  ...
  const tasks = trustRes.rows.map((row: any) => {
...
async function loadNotifications(
  ...
  const signupState: FilterState = { conditions: [], params: [], idx: 1 };
  appendScope(signupState, access, {
    country: 'u.country_id',
    division: 'u.division_id',
  });
  const signupWhere =
    signupState.conditions.length > 0
  ...
  const fundingState: FilterState = { conditions: [`e.status IN ('FUNDED', 'PARTIALLY_DISBURSED', 'COMPLETED')`], params: [], idx: 1 };
  ...
  const items = [
    ...
    ...fundedRes.rows.map((row: any) => ({
      id: `funded:${row.id}`,
      type: 'ESCROW_FUNDED',
      title: 'Campaign funded',
      body: `${normalizeText(row.business_name, 'A business')} funded "${normalizeText(
        row.campaign_title,
        'a campaign'
      )}".`,
      created_at: normalizeTimestamp(row.created_at),
...
async function loadMessages(client: PoolClient, limit: number) {
...
async function loadOperators(client: PoolClient, limit: number) {
  const capped = Math.min(Math.max(Math.trunc(limit), 10), 100);
  const res = await client.query(
    `
    SELECT
...
async function loadAuditFeed(
  ...
  const capped = Math.min(Math.max(Math.trunc(limit), 10), 100);
  const state: FilterState = { conditions: [], params: [], idx: 1 };
  appendScope(state, access, {
    country: 'country_id',
...
export async function ensureAdminOperationsSchema(client: PoolClient) {
  await client.query(`
...
export async function loadAdminOperationsSnapshot(
  client: PoolClient,
  access: DashboardAccessContext,
  options: AdminOperationsSnapshotOptions = {}
  ...
  const slaMinutes = parsePositiveSetting(
    settings.operations_sla_minutes,
    DEFAULT_SLA_MINUTES
  );
  ...
  const tasks = await safeLoad(
    partialErrors,
    'tasks',
    () => loadVisibleTasks(client, access, slaMinutes),
    [] as AdminOperationTaskRecord[]
  );
  const states = await safeLoad(
    partialErrors,
    'task_states',
    () => loadTaskStates(client, tasks.map((task) => task.task_key)),
    [] as AdminOperationTaskStateRow[]
  );
  ...
  const notifications = await safeLoad(
    partialErrors,
    'notifications',
    () => loadNotifications(client, access, notificationLimit),
  ...
  const messages =
    options.includeMessages === false
      ...
      : await safeLoad(
          partialErrors,
          'messages',
          () => loadMessages(client, messageLimit),
          [] as Array<Record<string, unknown>>
        );

  const operators =
    options.includeWorkforce === false
      ? []
      : await safeLoad(
          partialErrors,
          'operators',
          () => loadOperators(client, operatorLimit),
          [] as Array<Record<string, unknown>>
        );
  ...
  const auditFeed =
    ...
    (!hasAdminModuleAccess(access, ADMIN_MODULE_AUDIT_LOGS) &&
      ...
      : await safeLoad(
          partialErrors,
          'audit',
          () => loadAuditFeed(client, access, auditLimit),
          [] as Array<Record<string, unknown>>
        );
  ...
  return {
    ...
    settings: {
      ...
      operations_ack_minutes: String(
        parsePositiveSetting(
          settings.operations_ack_minutes,
          DEFAULT_ACK_MINUTES,
          slaMinutes
        )
      ),
...
export async function loadVisibleAdminOperationTask(
  client: PoolClient,
  access: DashboardAccessContext,
  taskKey: string
) {
...
export async function claimAdminOperationTask(
  client: PoolClient,
  actorUserId: string,
  task: AdminOperationTaskInput,
  ackMinutes: number
) {
  await ensureAdminOperationsSchema(client);
  const res = await client.query(
...
export async function acknowledgeAdminOperationTask(
  client: PoolClient,
  actorUserId: string,
  task: AdminOperationTaskInput,
  ackMinutes: number
...
export async function loadAdminOperationTaskState(
  client: PoolClient,
  taskKey: string
) {
...
export async function releaseAdminOperationTaskClaim(
  client: PoolClient,
  taskKey: string
) {
  await ensureAdminOperationsSchema(client);
...
export async function resolveAdminOperationTaskByEntity(
  client: PoolClient,
  entityType: string,
  entityId: string,
...
export async function createAdminOperationMessage(
  client: PoolClient,
  senderUserId: string,
  body: string
) {
