...
export type BusinessProPlanCode = keyof typeof BUSINESS_PRO_PLANS;
 ...
 */
export function proKeepAliveConditionSql(businessIdSqlExpr: string): string {
  return `EXISTS (
...
export async function ensureBusinessProSubscriptionSchema(client: any) {
  await client.query(`
...
export async function getLatestBusinessProSubscription(
  client: any,
  businessId: string
) {
...
export async function getBusinessProSubscriptionStatus(
  client: any,
  businessId: string
) {
  ...
  const validUntil =
    row?.valid_until == null ? null : new Date(row.valid_until as string);
  const isActive =
    row != null &&
    validUntil != null &&
...
export async function hasActiveBusinessProSubscription(
  client: any,
  businessId: string
) {
...
export async function reserveBusinessProSubscription(
  client: any,
  businessId: string,
  paymentReference: string,
  planCode: BusinessProPlanCode = 'monthly'
) {
  await ensureBusinessProSubscriptionSchema(client);
...
export async function markBusinessProSubscriptionPaid(
  client: any,
  businessId: string,
  paymentReference: string
) {
