...
export async function paymentRoutes(app: FastifyInstance) {
  ...
  app.post('/payments/pesapal/ipn', async (request, reply) => {
    ...
    const statusInfo = await getTransactionStatus(eventId, merchantReference);
    ...
    const result = await withTransaction(async (client) => {
      ...
      const amount = parseInt(statusInfo.amount ?? statusInfo.Amount ?? '0', 10);
      ...
      const status = (statusInfo.payment_status_description ?? statusInfo.status ?? '').toUpperCase();
