...
import { submitOrder, getTransactionStatus, } from '../services/pesapal.js';
export async function paymentRoutes(app) {
    const paymentRepo = new PaymentRepo();
    // ============================================
    // CREATE ESCROW ORDER
    // ============================================
    app.post('/api/payments/create-escrow-order', { preHandler: app.authenticate }, async (request, reply) => {
        try {
            ...
            const order = (await submitOrder({
                ...
                callback_url: 'https://status-influence-marketing-production.up.railway.app/payment/success',
                cancellation_url: 'https://status-influence-marketing-production.up.railway.app/payment/cancel',
            }));
    ...
    app.post('/payments/pesapal/ipn', async (request, reply) => {
        ...
        const orderTrackingId = body?.OrderTrackingId ?? body?.orderTrackingId ?? body?.id;
        ...
        setImmediate(async () => {
            try {
                const statusInfo = (await getTransactionStatus(String(orderTrackingId), String(merchantReference)));
                await withTransaction(async (client) => {
                    ...
                    const amount = typeof amountRaw === 'string'
                        ...
                        : Number(amountRaw ?? 0);
                    if (amount !== escrow.amount_total)
                        return;
                    const status = String(statusInfo.payment_status_description ??
                        statusInfo.status ??
                        '').toUpperCase();
                    if (status.includes('COMPLETED') || status.includes('SUCCESS')) {
                        await paymentRepo.updatePesaPalTxnStatus(client, merchantReference, 'COMPLETED', String(orderTrackingId));
                        await paymentRepo.markEscrowFunded(client, escrow.id, txn.id);
                    }
                    else if (status.includes('FAILED')) {
                        await paymentRepo.updatePesaPalTxnStatus(client, merchantReference, 'FAILED', String(orderTrackingId));
                    }
                });
            }
            catch (error) {
