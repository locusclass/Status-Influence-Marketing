...
export function estimateViewsForBudget(budgetUgx: number): number {
  return Math.floor(budgetUgx / PUBLIC_CONTRACT_RATE_UGX);
}

export function getCampaignPackage(code: string): CampaignPackage | undefined {
...
export const MARKETPLACE_LISTING_PLANS: MarketplaceListingPlan[] = [
  {
    code: 'BASIC',
    label: 'Basic Listing',
...
export function getMarketplaceListingPlan(code: string): MarketplaceListingPlan | undefined {
