...
export function canAccessAdminDashboard(value: unknown) {
...
export function calculateRevenueBreakdown(
  ...
  const netPlatformRevenue = roundMoney(
    platformFee - countryShare - divisionShare
  );
...
export async function recordCampaignRevenueEntry(
  client: DbClientLike,
  campaignId: string
) {
  ...
  const breakdown = calculateRevenueBreakdown(
    Number(campaign.gross_amount ?? 0),
    Boolean(campaign.division_id)
  );
