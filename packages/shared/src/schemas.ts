...
export const SubmitProofSchema = z.object({
  ...
  proof_video_url: z.string().trim().min(1).max(2048),
...
export const CampaignBundleItemSchema = z
  .object({
    ...
    execution_mode: z.enum(['PRIVATE_CONTRACT', 'PUBLIC_ADVERT']).optional(),
    ...
    start_date: z.string(),
    end_date: z.string(),
    public_ad_viewer_target: z.number().int().min(1).optional(),
...
export const CreateCampaignSchema = z
  ...
  .superRefine((value, ctx) => {
    const hasBundleItems =
      Array.isArray(value.bundle_items) && value.bundle_items.length > 0;
    const hasSharedPlatforms =
    ...
    if (
      ...
      const bundlePlatforms = new Set(
        value.bundle_items.map((item) => item.platform)
      );
      const declaredPlatforms = new Set(value.platforms);
      if (
        bundlePlatforms.size !== declaredPlatforms.size ||
